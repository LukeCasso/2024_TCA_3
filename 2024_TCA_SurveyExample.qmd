...
survey_data <- survey_data[-1, , drop = FALSE]
...
recommend <- as.numeric(survey_data$Recommendation.Likelihood)
hist(recommend,
...
boxplot(recommend ~ num_Atmosphere,
        survey_data,
        na.action = na.omit,
        col = "lightblue",
        outline = TRUE,
...
results_sw <- shapiro.test(recommend)
print(results_sw)
...
cor_results <- cor.test(hours_played, recommend, method = "pearson")
```
